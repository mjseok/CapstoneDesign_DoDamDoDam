...
const SimpleModal = (props) => {
  ...
  useEffect(() => {
    ...
    setValue((value) => {
      ...
      corrections.forEach((correction, i) => {
        str = str.replace(correction.token, correction.isCorrected ? `${correction.suggestions[0]}` : `<span class="error" data-index=${i}>${correction.token}</span>`);
      });
      return str;
    });
  }, [corrections]);
  ...
  const handleSubmit = useCallback(
    (value) => {
      if (corrections && corrections.filter((x) => !x.isCorrected).length > 0) return alert("맞춤법을 전부 수정하셔야 제출할 수 있습니다.");
  ...
  return (
    <>
      ...
      <Modal style={{ maxWidth: "800px", width: "90%" }} isOpen={modal} toggle={toggle}>
        <ModalHeader toggle={toggle}>
          <h2>기백이의 2월 19일 일기</h2>
        </ModalHeader>
        <ModalBody>
          <div>
            <EditArea>
              <ContentEditable className={classes.textarea} html={value} onChange={handleChange} />
              <CorrectionArea>
                {!isEmpty(filteredCorrections) && <CorrectionTitle>아래와 같이 변경해주세요.</CorrectionTitle>}
            ...
            <div>{transcript}</div>
            <img id="img1" src="https://i.ibb.co/t48ps4X/microphone.png" className={classes.microphoneIcon} onClick={changeImage} />
          </div>
        </ModalBody>
        <ModalFooter>
          <div>
            ...
            {isChecked && (
              <Button type="button" color="primary" onClick={() => handleSubmit(value + speechValue)} style={{ marginLeft: "16px" }}>
                일기 제출하기
              </Button>
            )}
          </div>
